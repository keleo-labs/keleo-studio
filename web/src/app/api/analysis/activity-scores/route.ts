...
import { calculateActivitySpaceScores } from "@/lib/analysis/methodFocus";
 ...
 *
 * Calculate activity space coverage scores for a practice or method.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Calculate activity space scores
    const activityScoresMap = calculateActivitySpaceScores(doc, baseline, grouped);

    // Convert Map to plain object for JSON serialization
    const scoresByFocus: Record<string, any> = {};
    activityScoresMap.forEach((value, key) => {
    ...
    const message = e instanceof Error ? e.message : "Activity score calculation failed";
    console.error("Activity scores error:", e);
    return NextResponse.json(
      { error: message },
      { status: 500 }
    );
